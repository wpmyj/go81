...
{
    public class 办事指南Controller : Controller
    {
        private static int PAGESIZE = int.Parse(ConfigurationManager.AppSettings["下载每页显示条数"]);
        //
        // GET: /下载/
        public ActionResult ServiceGuideList()
        {
            return View();
        }
        public ActionResult Part_ServiceGuideList(int? page)
        {
            int p = 1;
            try
            {
            ...
            int listcount = (int)(办事指南管理.计数办事指南(0, 0,null, false));
            ...
            ViewData["办事指南列表"] = 办事指南管理.查询办事指南(PAGESIZE * (p - 1), PAGESIZE,null, false, SortBy.Descending("内容主体.发布时间"), includeDisabled: false);
            return PartialView("Part_Service/Part_ServiceGuideList");
        }
        public ActionResult ServiceDetail()
        {
            return View();
        }
        public ActionResult Part_ServiceDetail()
        {
            办事指南 model = null;
            try
            {
                model = 办事指南管理.查找办事指南(int.Parse(Request.QueryString["id"]));
            }
            catch
            {

            }
            return PartialView("Part_Service/Part_ServiceDetail", model);
        }
        ...
        public ActionResult Part_SearchByPage()
        {
            ...
            ViewData["办事指南列表"] = 办事指南管理.查询办事指南(PAGESIZE * (page - 1), PAGESIZE,null, false, SortBy.Descending("内容主体.发布时间"), false);
            return PartialView("Part_Service/Part_SearchByPage");
        }

        public ActionResult Part_OtherList()
        {
            ViewData["最近办事指南"] = 办事指南管理.查询办事指南(0, 10,null, false, SortBy.Descending("内容主体.发布时间"));
            return PartialView("Part_Service/Part_OtherList");
        }
