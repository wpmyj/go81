...
namespace Go81WebApp.Models.管理器.需求计划管理
{
    public class 需求计划管理
    {
        ...
        public static bool 屏蔽需求计划(long id)
        {
            return Mongo.屏蔽<需求计划>(id);
        }
        public static bool 屏蔽需求计划解除(long id)
        {
            return Mongo.屏蔽解除<需求计划>(id);
        }
        public static bool 删除需求计划(long id)
        {
            return Mongo.删除<需求计划>(id);
        }
        public static 需求计划 查找需求计划(long id)
        {
            return Mongo.查找<需求计划>(id);
        }
        /// <summary>
        ...
        public static IEnumerable<需求计划> 查询需求计划(int skip, int limit, IMongoQuery conditions = null, bool modifiedDescending = true, IMongoSortBy sorting = null, bool includeDisabled = true, bool includeDeleted = false)
        {
            return Mongo.查询<需求计划>(skip, limit, conditions, modifiedDescending, sorting, includeDisabled, includeDeleted);
        }
        /// <summary>
        ...
        /// <param name="includeDeleted">查找范围包括标记为【已删除】的记录。默认不包括。</param>
        /// <returns>特定类型的可枚举对象，用 foreach 遍历</returns>
        public static long 计数需求计划(int skip, int limit, IMongoQuery conditions = null, bool includeDisabled = true, bool includeDeleted = false)
        {
            return Mongo.计数<需求计划>(skip, limit, conditions, includeDisabled, includeDeleted);
        }
    }
}