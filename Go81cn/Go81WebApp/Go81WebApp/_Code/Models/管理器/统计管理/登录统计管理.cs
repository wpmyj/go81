...
namespace Go81WebApp.Models.管理器.统计管理
{
    public static class 登录统计管理
    {
        ...
        public static bool 屏蔽登录统计(long id)
        {
            return Mongo.屏蔽<登录统计>(id);
        }
        public static bool 屏蔽登录统计解除(long id)
        {
            return Mongo.屏蔽解除<登录统计>(id);
        }
        public static bool 删除登录统计(long id)
        {
            return Mongo.删除<登录统计>(id);
        }
        public static 登录统计 查找登录统计(long id)
        {
            return Mongo.查找<登录统计>(id);
        }
        /// <summary>
        ...
        public static IEnumerable<登录统计> 查询登录统计(int skip, int limit, IMongoQuery conditions = null, bool modifiedDescending = true, IMongoSortBy sorting = null, bool includeDisabled = true, bool includeDeleted = false)
        {
            return Mongo.查询<登录统计>(skip, limit, conditions, modifiedDescending, sorting, includeDisabled, includeDeleted);
        }
        /// <summary>
        ...
        /// <param name="includeDeleted">查找范围包括标记为【已删除】的记录。默认不包括。</param>
        /// <returns>特定类型的可枚举对象，用 foreach 遍历</returns>
        public static long 计数登录统计(int skip, int limit, IMongoQuery conditions = null, bool includeDisabled = true, bool includeDeleted = false)
        {
            return Mongo.计数<登录统计>(skip, limit, conditions, includeDisabled, includeDeleted);
        }
    }
}