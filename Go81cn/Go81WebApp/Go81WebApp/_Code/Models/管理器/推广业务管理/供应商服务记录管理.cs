...
namespace Go81WebApp.Models.管理器.推广业务管理
{
    public class 供应商服务记录管理
    {
        ...
        public static bool 屏蔽供应商服务记录(long id)
        {
            return Mongo.屏蔽<供应商服务记录>(id);
        }
        public static bool 屏蔽供应商服务记录解除(long id)
        {
            return Mongo.屏蔽解除<供应商服务记录>(id);
        }
        public static bool 删除供应商服务记录(long id)
        {
            return Mongo.删除<供应商服务记录>(id);
        }
        public static 供应商服务记录 查找供应商服务记录(long id)
        {
            return Mongo.查找<供应商服务记录>(id);
        }
        /// <summary>
        ...
        public static IEnumerable<供应商服务记录> 查询供应商服务记录(int skip, int limit, IMongoQuery conditions = null, bool modifiedDescending = true, IMongoSortBy sorting = null, bool includeDisabled = true, bool includeDeleted = false)
        {
            return Mongo.查询<供应商服务记录>(skip, limit, conditions, modifiedDescending, sorting, includeDisabled, includeDeleted);
        }
        ...
        /// <param name="includeDeleted">查找范围包括标记为【已删除】的记录。默认不包括。</param>
        /// <returns>特定类型的可枚举对象，用 foreach 遍历</returns>
        public static long 计数供应商服务记录(int skip, int limit, IMongoQuery conditions = null, bool includeDisabled = true, bool includeDeleted = false)
        {
            return Mongo.计数<供应商服务记录>(skip, limit, conditions, includeDisabled, includeDeleted);
        }
    }
}